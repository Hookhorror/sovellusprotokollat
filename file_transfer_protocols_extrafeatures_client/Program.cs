...
namespace file_transfer_protocols_extrafeatures_client
{
    class Program
    {
        ...
        private static bool CheckForCorrectAck(Socket soc, ref EndPoint ep, int blockNumber)
        {
            byte[] expected = { (byte)0, (byte)4, (byte)(blockNumber / 256), (byte)(blockNumber % 256) };
            byte[] rec = new byte[4];
            int howMany = soc.ReceiveFrom(rec, 0, ref ep);
        ...
        private static void SendFileTo(IPAddress host, Socket soc, string fileName)
        {
            ...
            using (FileStream fs = File.OpenRead(@"./" + fileName))
            {
                do
                {
                    ...
                    ackIsCorrect = CheckForCorrectAck(soc, ref remote, blockNumber);
                    ...
                    {
                        soc.SendTo(lastBlockSent, remote);
                    }
